...
describe('departmentController incremental migration -> centralized errorHandler', () => {
    ...
    it('delegates update missing-resource flow to centralized handler', async () => {
        ...
        await departmentController.updateDepartment(req, res, (error) => {
            capturedError = error;
        });
    ...
    it('delegates update duplicate-code conflict to centralized handler', async () => {
        vi.spyOn(Department, 'findByPk').mockResolvedValue({
            code: 'ANT',
            slug: 'antioquia'
        });
        vi.spyOn(Department, 'findOne').mockResolvedValueOnce({ id: '22', code: 'BOG' });
    ...
    it('delegates update duplicate-slug conflict to centralized handler', async () => {
        vi.spyOn(Department, 'findByPk').mockResolvedValue({
            code: 'ANT',
            slug: 'antioquia'
        });
        vi.spyOn(Department, 'findOne')
            .mockResolvedValueOnce(null)
            .mockResolvedValueOnce({ id: '22', slug: 'bogota' });
