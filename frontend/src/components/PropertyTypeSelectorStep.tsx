...
import PrePublicationModal, { getDismissedTypes } from './PrePublicationModal';
...
const PropertyTypeSelectorStep: React.FC<PropertyTypeSelectorStepProps> = ({ onSelect, selectedType }) => {
    ...
    // pendingType: tipo seleccionado pero pendiente de confirmación en el modal
    const [pendingType, setPendingType] = useState<PropertyType | null>(null);
    ...
    /**
     * Called when the user clicks a property type card.
     * If the user previously dismissed the modal for this type, skip it.
     * Otherwise, open the PrePublicationModal first.
     */
    const handleCardClick = (type: PropertyType) => {
        const dismissed = getDismissedTypes();
        if (dismissed.includes(type)) {
            // User already knows the checklist → go directly to the form
            onSelect(type);
        } else {
            // Show the pre-publication recommendations modal
            setPendingType(type);
        }
    };
    ...
    return (
        <>
            ...
            {/* Pre-Publication Recommendations Modal */}
            {pendingType && (
                <PrePublicationModal
                    propertyType={pendingType}
                    onConfirm={() => {
